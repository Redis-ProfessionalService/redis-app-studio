...
public class CreateLibraryTask implements Task
{
    private final String mRunName = "library";
    private final String mTestName = "library";
        ...
        throws FCException
    {
        ...
        mJavaSrcPathName = String.format("%s%csrc%cmain%cjava%c%s", mPkgRootPathName,
                                         ...
                                         File.separatorChar, File.separatorChar,
                                         javaClassPathName);
        mTestSrcPathName = String.format("%s%csrc%ctest%cjava%c%s", mPkgRootPathName,
                                         File.separatorChar, File.separatorChar,
                                         File.separatorChar, File.separatorChar,
                                         javaClassPathName);
        ...
        throws IOException
    {
        ...
        if (templateFile.exists())
        {
            ...
            for (File tmpFile : templateFileList)
            {
                ...
                if (StringUtils.endsWithIgnoreCase(fileName, "Test.java"))
                {
                    pkgPathFileName = String.format("%s%c%s", mTestSrcPathName, File.separatorChar, fileName);
                    aUtility.docFieldReplaceInTemplate(aDoc, tmpFile.getAbsolutePath(), pkgPathFileName);
                }
                else if (StringUtils.endsWithIgnoreCase(fileName, ".java"))
                {
