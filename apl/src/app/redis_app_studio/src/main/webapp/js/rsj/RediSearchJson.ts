    ...
    private rebuildSearchIndex(aResponse: string): void
    {
        if (aResponse == "OK")
        {
            ...
            if (schemaGrid != null)
            {
                ...
                setTimeout(() => { schemaGrid.invalidateCache(); schemaGrid.filterData(RediSearchJson.prototype.defaultCriteria(windowAppContext.getDSStructure(), windowAppContext.getAppViewTitle())); }, 2000);
            }
        }
    }
    ...
    private flushDatabase(aResponse: string): void
    {
        if (aResponse == "OK")
        {
            ...
            if ((redisDBInfoWindow != null) && (redisDBInfoForm != null))
            {
                ...
                // Allow time for the index rebuild and notification to complete before schema refresh
                setTimeout(() => { RediSearchJson.prototype.executeAppViewGridSearch(); }, 2000);
            }
        }
    }
