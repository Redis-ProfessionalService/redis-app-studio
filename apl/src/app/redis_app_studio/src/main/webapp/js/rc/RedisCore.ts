    ...
    private rebuildDatabase(aResponse: string): void
    {
        if (aResponse == "OK")
        {
            ...
            if (schemaGrid != null)
            {
                ...
                // setTimeout(() => { schemaGrid.invalidateCache(); schemaGrid.filterData(RedisCore.prototype.defaultCriteria(windowAppContext.getDSStructure(), windowAppContext.getAppViewTitle())); }, 2000);
            }
        }
    }
    ...
    private flushDatabase(aResponse: string): void
    {
        if (aResponse == "OK")
        {
            ...
            if ((redisDBInfoWindow != null) && (redisDBInfoForm != null))
            {
                ...
                // Allow time for the index rebuild and notification to complete before schema refresh
                setTimeout(() => { RedisCore.prototype.executeAppViewGridFetch(); }, 2000);
            }
        }
    }
