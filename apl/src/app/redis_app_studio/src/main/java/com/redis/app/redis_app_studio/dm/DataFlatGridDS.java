...
public class DataFlatGridDS extends BasicDataSource
{
	private final String CLASS_NAME = "DataFlatDS";
...
// We use a CSV file to manage the document list in a folder.
		...
		DataLoader dataLoader = new DataLoader(appCtx, Constants.DS_DATA_STRUCTURE_FLAT_NAME);
		String storageCSVPathFileName = dataLoader.deriveStoragePathFileName(Constants.DS_DATA_FLAT_PATH_NAME, Constants.DS_STORAGE_DETAILS_NAME);
...
// Get a handle to the application logger instance.
		...
		if (rowCount > 0)
		{
			...
			for (int row = 0; row < rowCount; row++)
			{
				...
				pathFileName = aDataLoader.deriveStoragePathFileName(Constants.DS_DATA_FLAT_PATH_NAME, fileName);
