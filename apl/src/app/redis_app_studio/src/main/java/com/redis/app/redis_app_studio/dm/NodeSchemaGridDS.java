...
public class NodeSchemaGridDS extends BasicDataSource
{
	private final String CLASS_NAME = "NodeSchemaGridDS";
...
// Execute the operation.
		...
		stopWatch.start();
		DataGrid dataGrid = graphDS.vertexSchemaDocToDataGrid(gridDS.getSchema(), false);
		stopWatch.stop();
...
// changes.
		...
		DataGrid schemaGrid = graphDS.vertexSchemaDocToDataGrid(gridDS.getSchema(), false);
		...
		StopWatch stopWatch = new StopWatch();
		stopWatch.start();
		boolean isOK = graphDS.updateVertexSchema(schemaDoc);
		stopWatch.stop();
		appLogger.debug(String.format("'%s': Updated graph vertex schema definition in %d milliseconds with an OK status of %s",
