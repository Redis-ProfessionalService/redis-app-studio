...
// Execute the operation.
		...
		stopWatch.start();
		DataGrid dataGrid = Data.schemaDocToDataGrid(gridDS.getSchema(), false);
		stopWatch.stop();
...
// changes.
		...
		StopWatch stopWatch = new StopWatch();
		stopWatch.start();
		boolean isOK = gridDS.updateSchema(schemaDoc);
		stopWatch.stop();
		appLogger.debug(String.format("'%s': Updated grid schema definition in %d milliseconds with an OK status of %s",
...
//		dataDocLogger.writeFull(gridDS.getSchema());
		...
		SCDSResponse scdsResponse = new SCDSResponse(appCtx, scDS);
		scdsResponse.setDebugFlag(false);
		DSResponse dsResponse = scdsResponse.create(schemaDoc, true);
