...
// Execute the operation.
		...
		RedisJson redisJson = appResource.getRedisJson();
		RedisSearch redisSearch = appResource.getRedisSearch();
		RedisKey redisKey = appResource.getRedisDS().getRedisKey();
...
// changes.
		...
		Optional<DataItem> optDataItem = updateDoc.getFirstItemByFeatureNameOptional(Data.FEATURE_IS_PRIMARY);
		if (optDataItem.isPresent())
		{
			DataItem dataItem = optDataItem.get();
			if (dataItem.isValueAssigned())
				keyName = redisKey.moduleJson().redisJsonDocument().dataObject(updateDoc).primaryId(dataItem.getValue()).name();
			else
				keyName = redisKey.moduleJson().redisJsonDocument().dataObject(updateDoc).name();
		}
		else
			keyName = redisKey.moduleJson().redisJsonDocument().dataObject(updateDoc).name();
