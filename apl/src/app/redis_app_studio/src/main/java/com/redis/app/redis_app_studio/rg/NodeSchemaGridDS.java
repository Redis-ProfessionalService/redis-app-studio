...
// Execute the operation.
		...
		if (optDataDoc.isPresent())
		{
			...
			dataGrid = redisGraph.schemaDocToDataGrid(nodeSchemaDoc, false);
		}
		else
...
// changes.
		...
		{
			DataDoc nodeSchemaDoc = optDataDoc.get();
			schemaGrid = redisGraph.schemaDocToDataGrid(nodeSchemaDoc, false);
		}
		else
			throw new DSException("The RedisGraph node schema was not found in database.");
		...
		stopWatch.stop();
		appLogger.debug(String.format("'%s': Updated graph vertex schema definition in %d milliseconds",
