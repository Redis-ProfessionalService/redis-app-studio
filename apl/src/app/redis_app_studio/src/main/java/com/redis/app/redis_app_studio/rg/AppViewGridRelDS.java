...
public class AppViewGridRelDS extends BasicDataSource
{
		...
		throws FCException
	{
		...
		{
			String msgStr = String.format("[%s]: Unable to locate value for '%s' item in data document.",
										  aSCDataDoc.getName(), Data.GRAPH_SRC_VERTEX_ID_NAME);
			appLogger.error(msgStr);
...
//		DataDocLogger dataDocLogger = new DataDocLogger(appLogger);
		...
		if (isOK)
		{
			String vertexLabel = srcDataDoc.getValueByName(Data.GRAPH_VERTEX_LABEL_NAME);
			if (StringUtils.isNotEmpty(vertexLabel))
			{
				srcDataDoc.setName(vertexLabel);
				srcDataDoc.setTitle(vertexLabel);
				srcDataDoc.remove(Data.GRAPH_VERTEX_LABEL_NAME);
//				appLogger.debug("Vector DataDoc Source Logger Dump");
//				dataDocLogger.writeSimple(srcDataDoc);
			}
			vertexLabel = dstDataDoc.getValueByName(Data.GRAPH_VERTEX_LABEL_NAME);
			if (StringUtils.isNotEmpty(vertexLabel))
			{
				dstDataDoc.setName(vertexLabel);
				dstDataDoc.setTitle(vertexLabel);
				dstDataDoc.remove(Data.GRAPH_VERTEX_LABEL_NAME);
//				appLogger.debug("Vector DataDoc Destination Logger Dump");
//				dataDocLogger.writeSimple(dstDataDoc);
			}
			String edgeType = scDataDoc.getValueByName(Data.GRAPH_EDGE_TYPE_NAME);
			if (StringUtils.isNotEmpty(edgeType))
			{
				scDataDoc.setName(edgeType);
				scDataDoc.setTitle(edgeType);
				scDataDoc.remove(Data.GRAPH_SRC_VERTEX_ID_NAME);
				scDataDoc.remove(Data.GRAPH_DST_VERTEX_ID_NAME);
				scDataDoc.remove(Data.GRAPH_VERTEX_NAME);
//				appLogger.debug("Edge DataDoc Logger Dump");
//				dataDocLogger.writeSimple(scDataDoc);
			}
			isOK = redisGraph.addRelationship(dataGraph, srcDataDoc, dstDataDoc, scDataDoc);
		}
		stopWatch.stop();
...
//				dataDocLogger.writeSimple(scDataDoc);
			...
			int propertiesUpdated = redisGraph.updateRelationship(dataGraph, scDataDoc);
			isOK = propertiesUpdated > 0;
		}
		stopWatch.stop();
		if (isOK)
